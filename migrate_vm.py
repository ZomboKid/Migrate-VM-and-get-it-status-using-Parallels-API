...
def migrate_vm(OLD_HOST, OLD_HOST_USER, OLD_HOST_PASSW, NEW_HOST,
               NEW_HOST_USER, NEW_HOST_PASSW, VM):
    prlsdkapi.init_server_sdk()
    old_server = prlsdkapi.Server()
    old_server.login(OLD_HOST, OLD_HOST_USER, OLD_HOST_PASSW, '',
                     0, 0, consts.PSL_NORMAL_SECURITY).wait()

    vm = get_vm(old_server, VM)

    new_server = prlsdkapi.Server()
    new_server.login(NEW_HOST, NEW_HOST_USER, NEW_HOST_PASSW, '',
                     0, 0, consts.PSL_NORMAL_SECURITY).wait()

    print "--------------------------------------------"
    print "Migration vm", VM, "from", OLD_HOST, "to", NEW_HOST,\
          "is in progress..."
...
def main():
    migrate_vm(old_host_ip, old_host_user, old_host_passwd,
               new_host_ip, new_host_user, new_host_passwd, vm_name)
# ----------------------------------------------------------------------------
